 ...
 <section class="bg-light page-section" id="portfolio">
    ...
    <div class="row">
      <div class="col-lg-12 text-center" data-aos="zoom-in">
        <br>
        <h3 class="section-subheading text-muted">회원전체정보</h3>
  ...
  <table border="3" class="container">
  	...
  	<c:choose>
  		<c:when test="${empty selectalldto}">
  			<tr>
  				<td colspan="7">회원 아무도 없음 ㄷㄷ</td>
  			</tr>
  		</c:when>
  		<c:otherwise>
  			<c:forEach var="selectalldto" items="${selectalldto}">
  				<tr>
  					<td>${selectalldto.user_no }</td>
  					<td>${selectalldto.user_id }</td>
  					<td>${selectalldto.user_name }</td>
  					<td>${selectalldto.user_phone }</td>
  					<td>${selectalldto.user_email }</td>
  					<td>${selectalldto.user_grade }</td>
  					<td>${selectalldto.user_enabled }</td>	
  				</tr>
  			</c:forEach>
  		</c:otherwise>
  	</c:choose>
